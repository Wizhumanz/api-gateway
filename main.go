...
func main() {
	...
	router.Methods("GET").Path("/webhook").HandlerFunc(getAllWebhookConnectionHandler)
	router.Methods("POST").Path("/bot").HandlerFunc(createNewBotHandler)
	router.Methods("POST").Path("/addwebhook").HandlerFunc(createNewWebhookConnectionHandler)
